...
import { SettingsDto } from "~/app/_dtos/settings-dto";
...
import { SettingsFormSource } from "~/app/_models/form/settings-form.model";
import { Settings } from "~/app/_models/settings.model";
    ...
    IBaseDtoEntityMapper<Settings, SettingsFormSource, SettingsDto>
{
  mapEntity(settingsFromServer: Settings): Settings {
    ...
    mappedSettings.businessName = settingsFromServer.businessName;
  ...
  mapFormSourceToDto(settingsFormSource: SettingsFormSource): SettingsDto {
    return new SettingsDto(settingsFormSource.businessName);
  }

  mapEntityToFormSource(settings: Settings): SettingsFormSource {
    return new SettingsFormSource(settings.businessName);
  }

  mapFormSourceToEntity(settingsFormSource: SettingsFormSource): Settings {
    const settings = new Settings();
    settings.businessName = settingsFormSource.businessName;
    return settings;
  }
}
